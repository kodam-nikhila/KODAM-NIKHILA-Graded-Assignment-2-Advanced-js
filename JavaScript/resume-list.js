...
const fillData = () => {
    ...
    education.innerHTML = `<ul>${Object.keys(currentResume['education']).map(education => `<li><b>${education}:</b> ${Object.keys(currentResume['education'][education]).map(eduDataKey => `<span> ${currentResume['education'][education][eduDataKey]}</span>`)}</li>`)}</ul>`
...
fetch('https://api.npoint.io/60b7cfebbb9adb3fb90f')
